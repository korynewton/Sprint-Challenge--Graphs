...
# get exits for initial room
exits = player.currentRoom.getExits()

# initialize allexit options to '?'
graph[player.currentRoom.id] = {e: "?" for e in exits}

# starting from initial room, randomly select which of the available exits to try
...
while len(graph) < len(roomGraph):
    # save copy of current before making any moves
    ...
    # add move to traversalPath
    traversalPath.append(direction)

    # get exit paths of new room
    # exits = player.currentRoom.getExits()
    exits = {}
    # print(exits)

    # print('exits', player.currentRoom.getExits())
    # initialize graph for current room exits or set exits equal to the current state stored in graphs
    if player.currentRoom.id not in graph:
        graph[player.currentRoom.id] = {
            e: "?" for e in player.currentRoom.getExits()}
        exits = graph[player.currentRoom.id]
    else:
        exits = graph[player.currentRoom.id]

    print('should be in graph', graph[player.currentRoom.id])

    # update graph for current and previous
    graph[player.currentRoom.id] = exits

    graph[prev_room.id][direction] = player.currentRoom.id
    exits[opposite_direction[direction]] = prev_room.id

    # print('graph', graph[player.currentRoom.id])

    # dead_end = False
    unexplored_options = [e for e in graph[player.currentRoom.id]
                          if graph[player.currentRoom.id][e] is '?']
    # print('unexplored options', unexplored_options)

    # if no uneplored_options we;ve hit a dead end, time to BFS back 
    # to find a room with exit to explore
    if not unexplored_options:
    
        walk_back = set()
        q = Queue()
        # we'll need access to opposite direction to walk back
        # each tuple contains the direction and id to walk back to
        q.enqueue([(opposite_direction[direction], prev_room.id)])
        ...
        while q.size() > 0:
            ...
            # walk back
            if room_id not in walk_back:
                walk_back.add(room_id)
                ...
                for k, v in graph[room_id].items():
                    ...
                    if v == '?':
                        unexplored = k
                        break  # found new room to explore, break out
                    else:
                        # queue next paths to check
                        copy = list(path)
                        copy.append((k, v))
                        q.enqueue(copy)

                # if we've found the first room that still has a ? in graph, we've found the next
                # room to search down
                if unexplored:
                    # once we've found the closest unexplored room, we back back through rooms
                    # to get there
                    for info in path:
                        player.travel(info[0])
                        traversalPath.append(info[0])
                    direction = unexplored
                    break
    else:
        direction = random.choice(unexplored_options)
        # print('new direction', direction)
print(traversalPath)

# update prev_room

# while len(traversalPath) < len(roomGraph):
#     # while count < 5:
#         # remove last item
#     prev_room = s.pop()
